...
void SysTick_Handler(void) {
  ...
  if (i == 1000) {
    if (encoder.is_initialized){
      ...
      // output = (uint32_t)PID_Position_Calc(&pid, position);
      // Motor_SetOutput(&motor, output);

      // Increasemental PID update
      Encoder_Update(&encoder);
      speed = Encoder_GetSpeed(&encoder);
      output = (uint32_t)PID_Increasement_Calc(&pid, speed);
      Motor_SetOutput(&motor, output);
